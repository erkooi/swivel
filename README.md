...
![Swivel model for forward flight](pictures/swivel_0_side_zx_mid.jpg)

![Swivel model for hover flight](pictures/swivel_4_alpha_is_90_output.jpg)

## 3. Swivel movement as function of input tube rotation and mid tube rotation
### 3.1 Exact equations for swivel control
The matrix equations that yield the swivel output position, are available as function SwivelOutputPosition() in both swivel_functions.scad and swivel_functions.py. These matrix equations are expanded into gonio formules in SwivelOutputPosition_Gonio(), that is also available in both swivel_functions.scad and swivel_functions.py. This expansion yields a huge formula for the swivel output position as function of swivel dimensions, the rotation of the swivel input tube (phi_input) and the swivel mid tube (theta_mid).
...
![phi_output_yz(theta_mid)](pictures/phi_output_yz_as_function_of_theta_mid.jpg)
...
![theta_output_zx(theta_mid)](pictures/theta_output_zx_as_function_of_theta_mid.jpg)
...
The dft.scad implements the real input DFT in OpenSCAD. The implementation uses matrix multiplication and is not optimized like an Fast Fourier Transform (FFT). However with dft.scad it is possible to calculate the harmonic approximation of the swivel control entirely in OpenSCAD.
...
- The swivel is not suitable for thrust vectoring near horizontal during forward flight, because that would require full circle control and instantaneous control of the input tube orientation.
